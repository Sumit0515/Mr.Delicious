...
export default function Signup() {
  ...
  const handleClick = async (e) => {
    ...
    let latlong = await navLocation().then(res => {
      ...
      console.error(err);
      alert('Geolocation access denied. Please enter your address manually.');
      return null;
    });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const location = credentials.geolocation || address;
    if (!location) {
      alert("Please provide your location either by fetching it or entering it manually.");
      return;
    }
    ...
    if (json.success) {
      ...
      alert(`Error: ${json.errors.map(err => err.msg).join(', ')}`);
    }
  };
      ...
      <div className='container'>
        <form className='w-50 m-auto mt-5 border bg-dark border-success rounded' onSubmit={handleSubmit}>
          ...
          <div className="m-3">
            ...
            <fieldset>
              <input type="text" className="form-control" name='address' placeholder='Enter your address' value={address} onChange={(e) => setAddress(e.target.value)} aria-describedby="emailHelp" required />
            </fieldset>
          </div>
          <div className="m-3">
